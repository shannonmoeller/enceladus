...
defineElement('enc-game', (el) => {
  const { canvasEl, leftEl, rightEl, pauseEl, deathsEl, distanceEl, timeEl } = refs(el);

  let mapData = window.opener?.location?.origin === location.origin && window.opener?.editor?.file?.get();
